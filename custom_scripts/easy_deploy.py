...
class Operation(object):
    ...
    def stack_id(self):
        if self._stack_id is None:
            ...
            for stack in stacks:
		log("Stack found {0}".format(stack['Name']))
    ...
    def layer_rolling(self, comment, appVersion):
        load_balancer_name = self._get_opsworks_elb_name()
        args = {"AppSource" : {"Revision": appVersion} }
	try:
		self._make_api_call('opsworks', 'UpdateApp', app_id=self.application_id, **args)
	except Exception: 
  		log("response was {0} ")
	...
	healthy_attempts = 0; 
	healthy = self._is_instance_healthy(kwargs['load_balancer_name'], kwargs['ec2_instance_id'])
        while (healthy == False and healthy_attempts < 10):
		log("Waiting instance to be available to continue")
		time.sleep(5)
		healthy = self._is_instance_healthy(kwargs['load_balancer_name'], kwargs['ec2_instance_id'])
...
@click.option('--comment', help='Deployment message')
@click.option('--timeout', default=None, help='Deployment timeout')
@click.option('--appversion', required=True, help='OpsWorks Application Version')
@click.pass_context
def rolling(ctx, stack_name, layer_name, comment, timeout, appversion):
    operation = ctx.obj['OPERATION']
    operation.init(stack_name=stack_name, layer_name=layer_name, timeout=timeout)
    operation.layer_rolling(comment=comment, appVersion=appversion)
